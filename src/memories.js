...
function parseOutReasoning(result) {
    ...
    const answer = result.pipe.match(/<answer>(.*?)<\/answer>/gsv);

    if (answer && answer.length > 0) {
        result.pipe = answer[answer.length - 1];
        return;
    }

    const regex = new RegExp(`${prefix}(.*)${suffix}(\\n)*\\s*`, "svg");
    result.pipe = result.pipe.replace(regex, "");
    result.pipe = result.pipe.replace(/.*<\/reasoning>(\\n)*\s*/gsv, "");
    result.pipe = result.pipe.replace(/<[\/]*answer>(\\n)*\s*/gsv, "");
}
...
async function promptInfoBooks() {
    ...
    if (persona) {
        ...
        if (powerUserSettings.persona_descriptions[user_avatar]?.lorebook) {
            ...
            books[persona] =
                powerUserSettings.persona_descriptions[user_avatar].lorebook;
        }
    }
    ...
    let bookchars = await context.executeSlashCommandsWithOptions(
        `/buttons labels=${JSON.stringify(bookOptions)} multiple=true Which characters do you want to remember this?`,
    );
    let result = [];
...
async function processMessageSlice(mes_id, count = 0, start = 0) {
    ...
    message_history = await Promise.all(
        message_history.map(async (message, index) => {
            let placement = message.is_user
                ? regex_placement.USER_INPUT
                : regex_placement.AI_OUTPUT;
            let options = { isPrompt: true, depth: length - (start + index) - 1 };
            // no point in running the regexing on hidden messages
            let mes_text = message.is_system
                ? message.mes
                : getRegexedString(message.mes, placement, options);
            return {
                ...message,
                mes: mes_text,
                index: start + index,
    ...
    message_history = message_history.filter((it) => {
        return !it.is_system;
    });
    if (count > 0) {
        count++;
...
async function swapProfile() {
    ...
    if (current != settings.profile) {
        ...
        if (profile_list.findIndex((p) => p.id === settings.profile) < 0) {
            toastr.warning(
                "Invalid connection profile override; using current profile.",
                "ReMemory",
            );
            return false;
        }
...
async function generateMemory(message) {
    ...
    const memory_history = await processMessageSlice(
        mes_id,
        settings.memory_span,
    );
    debug("memory history", memory_history);
    const memory_context = memory_history
        .map((it) => `${it.name}: ${it.mes}`)
        .join("\n\n");
...
async function generateSceneSummary(mes_id) {
    ...
    let last_end = chat
        .slice(0, mes_id + 1)
        .findLastIndex((it) => it.extra.rmr_scene);
    if (last_end < 0) {
        last_end = 0;
    }
    ...
    if (chunks.length == 1) {
        ...
        toastr.info(
            `Generating summaries for ${chunks.length} chunks....`,
            "ReMemory",
        );
        let chunk_sums = [];
        let cid = 0;
        while (cid < chunks.length) {
            ...
            if (chunk_sum.length > 0) {
                ...
                const result = await getContext().Popup.show.text(
                    "ReMemory",
                    "There was an error generating a summary for chunk #" +
                    Number(cid) +
                    1,
        ...
        if (settings.add_chunk_summaries) {
            await runSlashCommand(
                `/comment at=${mes_id + 1} <details class="rmr-summary-chunks"><summary>Chunk Summaries</summary>${final_context}</details>`,
            );
        }
    } else {
...
export async function rememberEvent(message, options = {}) {
    ...
    if ("keywords" in options)
        keywords = options.keywords.split(",").map((it) => it.trim());
...
export async function endScene(message, options = {}) {
    ...
    if (mode !== SceneEndMode.NONE) {
        ...
        if (mode === SceneEndMode.MEMORY) {
            ...
            mes_id += 1;
            await runSlashCommand(
                `/comment at=${mes_id} ${summary} || /chat-jump ${mes_id}`,
            );
        }
    }
    ...
    toggleSceneHighlight(
        $(`.mes[mesid="${mes_id}"] .rmr-button.fa-circle-stop`),
        mes_id,
    );
